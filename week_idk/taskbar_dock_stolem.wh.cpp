...
void ApplyAnimation(double mouseX, DockAnimationContext& ctx) {
    ...
    // 1. Calculate scales and expansion for each icon based on mouse distance
    ...
    for (size_t i = 0; i < ctx.icons.size(); i++) {
        ...
        if (!tg || tg.Children().Size() < 2) continue;

        auto scale = tg.Children().GetAt(0).try_as<ScaleTransform>();
        auto translate = tg.Children().GetAt(1).try_as<TranslateTransform>();

        if (!scale || !translate) continue;
        ...
        // Apply shift (centering the whole expansion)
...
void RefreshIconPositions(DockAnimationContext& ctx) {
    ...
    // Initializes a 2-part transform chain for each icon: scale + translate.
    auto processButtonElement = [&](FrameworkElement const& element) {
        try {
            auto tg = element.RenderTransform().try_as<TransformGroup>();
            bool needsRebuild = true;

            if (tg && tg.Children().Size() >= 2) {
